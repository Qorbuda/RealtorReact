...
function Carousel(imagePath) {
    ...
    useEffect(() => {
        axios.get(`https://localhost:44394/api/Image/get-image-list?folderPath=${imagePath.imagePath}`)
            .then(response => {
            ...
            (
                <BootstrapCarousel interval={null} indicators={false}>
                    {imageUrls.map((item, index) => (
                        <BootstrapCarousel.Item>
                            <img src={`https://localhost:44394/api/image/${item}`} className="d-block w-100" alt={`Image ${index + 1}`} style={{borderRadius: "30px 30px 0px 0px", maxHeight:"286px", minHeight:"286px", minWidth:"384px"}} />
                        </BootstrapCarousel.Item>
                    ))}
                </BootstrapCarousel>
            )}
